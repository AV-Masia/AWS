...
> **Статус на 20.08.2026: инфраструктура погашена.** Задания показаны заказчику, и всё, что
> тарифицируется, снесено командой `terraform destroy`, чтобы не жечь кредиты вхолостую.
> Страница демо остаётся на месте, но входы на ней отвечать не будут: пула больше нет.
> Поднять всё заново — `cd infra && terraform apply` (около 7 минут), затем засеять базу
> `cd scripts && node seed.mjs` и подставить новые `terraform output` в `web/.env.production`.
> Идентификаторы пула и клиента при новом развёртывании будут другими — это нормально.
>
> Фактический расход за три дня работы демо: **$0,83**, целиком покрыто кредитами, с карты $0.
> Из них VPC endpoint $0,46, RDS $0,36, Secrets Manager $0,01. Показательно, что приватная
> «калитка» к Secrets Manager обошлась дороже самой базы данных.
...
| User Pool ID | `eu-central-1_F3JBMc9Ld` |
| App client ID | `jooc52jcone06i98bhme7anno` (public client, без секрета) |
| Домен страницы входа | `https://legacy-migration-931110358506.auth.eu-central-1.amazoncognito.com` |
| Callback / sign-out URL | `https://av-masia.github.io/AWS/` и `http://localhost:5173` |
| Scopes | `openid`, `email` |

**Пул один на оба задания.** Изначально для задания 1 пул создавался через консоль AWS мастером
`Create user pool → Define your application → Single-page application (SPA)` — он сразу делает
публичного клиента без секрета с grant type `Authorization code` и домен для страницы входа.
В задании 2 тот же пул был описан в Terraform, и держать рядом два почти одинаковых пула стало
незачем: консольный удалён, оба задания работают на Terraform-пуле. Что именно показывает консоль
на каждом шаге мастера — осталось в
[плане задания 1](docs/plans/cognito-idp-demo.md): требование «разобраться, какие бывают типы
аутентификации» закрывалось именно глазами в UI.

Одно отличие общего пула от того, что было в задании 1: в нём **включён** флоу
`ALLOW_USER_PASSWORD_AUTH`. Он нужен триггеру миграции — без пароля тот не сможет проверить
пользователя в старой базе. Когда все переедут, флоу выключается и остаётся SRP.
...
|---|---|---|
| `ALLOW_USER_SRP_AUTH` | Secure Remote Password — пароль не передаётся по сети даже в шифрованном виде, вместо него криптографическое доказательство знания | включён |
| `ALLOW_REFRESH_TOKEN_AUTH` | Обновление токенов без повторного входа | включён |
| `ALLOW_USER_PASSWORD_AUTH` | Пароль уходит на API в открытом виде (внутри TLS). Легаси, для миграций | **включён — нужен триггеру миграции** |
| `ALLOW_USER_AUTH` | Choice-based: пользователь выбирает способ — пароль, код на email, SMS, passkey. Основа passwordless-сценариев | выключен |
| `ALLOW_ADMIN_USER_PASSWORD_AUTH` | То же, что `USER_PASSWORD`, но серверным вызовом от имени администратора | выключен |
| `ALLOW_CUSTOM_AUTH` | Произвольная логика на Lambda-триггерах | выключен |

В консольном пуле задания 1 `ALLOW_USER_PASSWORD_AUTH` был выключен — так настроены умолчания
AWS, и это правильно для обычного приложения. Включить его пришлось ради миграции: SRP скрывает
пароль от всех, включая нашу Lambda, и проверить пользователя в старой базе с ним невозможно.
...
пароля на API. Умолчания настроены на более безопасные варианты, и включать его стоит только
с конкретной причиной. У нас причина есть — миграция, — и она временная.

Обе точки входа ведут в один пул, поэтому и триггеры работают на обеих: легаси-пользователь въедет
и через свою форму приложения, и через страницу входа Cognito
([таблица источников триггеров](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-working-with-lambda-triggers.html)
— `/login` в managed login вызывает `UserMigration_Authentication`). А вот honeypot работает только
в своей форме: страница Cognito не умеет передавать `clientMetadata`.
...
Пул поднят **целиком в Terraform**, консоль не использовалась. Он же обслуживает задание 1:
консольный пул удалён, чтобы не держать рядом две почти одинаковые сущности и не путать, где
что работает.
...
**Неочевидное:** миграция тоже проходит через этот триггер. Когда триггер миграции создаёт
пользователя, Cognito проводит его как созданного администратором и вызывает pre-signup
с источником `PreSignUp_AdminCreateUser` — это видно по таймстампам в CloudWatch: одна такая
запись через секунду после каждой успешной миграции. `clientMetadata` в этих событиях нет,
поэтому без фильтра по `triggerSource` honeypot блокировал бы **каждый** переезд пользователя.
