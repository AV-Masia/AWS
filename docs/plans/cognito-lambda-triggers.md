...
| Новый пул или import пула из задания 1 | **Новый пул в Terraform** (`legacy-migration-demo`), старый не трогаем | Import консольного пула — возня со сверкой атрибутов и риск сломать работающее демо задания 1. Требование «всё в IaC» относится к тому, что сдаём сейчас |
